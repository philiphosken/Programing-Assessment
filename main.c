...
int i; //Counter Index in order to read alter and reprint each element in the array
int k = 1; //KEY = the number of charcters shifted in the rotation cypher
...
for (i = 0 ; i < N ; ++i){
    ...
    if ((phi + k) > 90){ // If it exceeds 'Z' after k is added, then simply take 26 away then add k
